...
from rl_functions.policies import next_action_with_random_policy, \
    next_action_with_epsilon_random_policy, next_action_with_greedy_policy, \
...
def test_seeded_random_policy_gives_expected_results():
    ...
    for action_expected in actions_expected:
        action_actual = next_action_with_random_policy(N_CHOICES, random_generator=random_generator)
...
def test_two_random_policies_in_parallel_give_the_same_results():
    ...
    for _ in range(100):
        action_1 = next_action_with_random_policy(N_CHOICES, random_generator=random_generator_1)
        action_2 = next_action_with_random_policy(N_CHOICES, random_generator=random_generator_2)
...
def test_random_policy_gives_identical_results_to_epsilon_random_policy_when_epsilon_equals_one():
    ...
    action_actual = None
    for _ in range(100):
        action_expected = next_action_with_random_policy(N_CHOICES, random_generator=random_generator_1)
...
def test_epsilon_greedy_policy_gives_identical_results_to_random_policy_when_epsilon_equals_one():
    q_function = np.array([17.1, -5.5, 20.2, -20.9, 5.1, 1.0, -1.0, 1.5])
    random_generator_1 = default_rng(seed=0)
    random_generator_2 = default_rng(seed=0)
    for _ in range(100):
